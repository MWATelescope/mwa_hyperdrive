...
/// vis-simulate always generates timestamps starting from sched_start_gps,
/// ignoring the actual timestamps in an existing measurement set. When an MS
/// has been time-selected (e.g., by birli --sel-time), its data starts at a
/// later integration. Using vis-simulate to create model visibilities for
/// this MS produces model at the wrong timestamps, leading to incorrect UVW
/// coordinates and corrupted fringe phases.
///
/// The test MS 1090008640.ms contains a single timestep at GPS centroid
/// 1090008658 (18s after sched_start=1090008640). vis-simulate with 1
/// timestep produces centroid at sched_start + time_res/2 = 1090008644
/// (with default 8s time_res). The 14s discrepancy causes ~1.6m W-component
/// error on long baselines, translating to >1 radian phase error at 150 MHz.
///
/// cargo test --release -- cli::vis_simulate::tests::test_vis_simulate_start_time_offset --exact --show-output
#[test]
#[serial]
fn test_vis_simulate_start_time_offset() {
    let DataAsStrings {
        metafits, srclist, ..
    } = get_reduced_1090008640_raw();

    // Read the existing test MS to get its actual timestamp.
    let ms_path = PathBuf::from("test_files/1090008640/1090008640.ms");
    let metafits_path = PathBuf::from(&metafits);
    let ms_reader = MsReader::new(ms_path, None, Some(&metafits_path), None).unwrap();
    let obs_context = ms_reader.get_obs_context();
    let ms_centroid_gps = obs_context.timestamps.first().to_gpst_seconds();
    // Confirm the test MS is at GPS 1090008658 (18s after sched_start).
    assert_abs_diff_eq!(ms_centroid_gps, 1090008658.0);

    // Run vis-simulate with the same metafits, 1 timestep, output to MS.
    let temp_dir = TempDir::new().expect("couldn't make tmp dir");
    let output_path = temp_dir.path().join("model.ms");
    ...
    let sim_args = VisSimulateArgs::parse_from([
        ...
        "--source-list", &srclist,
        "--beam-type", "none",
        "--output-model-files", &output_str,
        "--num-timesteps", "1",
        "--num-fine-channels", "2",
        "--veto-threshold", "0.0",
    ]);
    let result = sim_args.run(false);
    assert!(result.is_ok(), "vis-simulate failed: {:?}", result.err().unwrap());

    // Read back the vis-simulate output MS.
    let sim_reader = MsReader::new(output_path, None, Some(&metafits_path), None).unwrap();
    let sim_context = sim_reader.get_obs_context();
    let sim_centroid_gps = sim_context.timestamps.first().to_gpst_seconds();

    // vis-simulate produces timestamp at sched_start + time_res/2
    // (= 1090008640 + 4 = 1090008644 with default 8s resolution),
    // not at the actual MS timestamp (1090008658).
    // The 14s Earth-rotation difference causes significant UVW errors
    // and corrupted fringe phases in the model visibilities.
    //
    // Desired behavior: vis-simulate should be able to produce model
    // visibilities at arbitrary timestamps matching an input MS.
    // Workaround: --time-offset 14 aligns the timestamps.
    assert_abs_diff_eq!(sim_centroid_gps, ms_centroid_gps, epsilon = 0.5);
}
