...
// exactly the same. `beam_args` are passed to both runs; if they're empty, the
// default (FEE) beam is used.
#[cfg(all(any(feature = "cuda", feature = "hip"), not(feature = "gpu-single")))]
fn cpu_gpu_match(beam_args: &[&str]) {
    use ndarray::prelude::*;
    ...
    #[rustfmt::skip]
    let mut argv = vec![
        "vis-simulate".to_string(),
        "--metafits".to_string(), metafits.display().to_string(),
        "--source-list".to_string(), args.srclist_args.source_list.unwrap(),
        "--output-model-files".to_string(), output_path.display().to_string(),
        "--num-timesteps".to_string(), format!("{num_timesteps}"),
        "--num-fine-channels".to_string(), format!("{num_chans}"),
    ];
    argv.extend(beam_args.iter().map(|s| s.to_string()));
    let mut cpu_argv = argv.clone();
    cpu_argv.push("--cpu".to_string());
    let sim_args = VisSimulateArgs::parse_from(cpu_argv);
    let result = sim_args.run(false);
    assert!(result.is_ok(), "result={:?} not ok", result.err().unwrap());
    ...
    let sim_args = VisSimulateArgs::parse_from(argv);
...
/// As above, but with the analytic beams. Unlike the tests that use beam
/// objects directly, this goes through the metafits, so the tiles have real
/// dipole delays and dead dipoles.
#[test]
#[serial]
#[cfg(all(any(feature = "cuda", feature = "hip"), not(feature = "gpu-single")))]
fn test_1090008640_vis_simulate_cpu_gpu_match_analytic_beams() {
    cpu_gpu_match(&["--beam-type", "analytic-mwa_pb"]);
    cpu_gpu_match(&["--beam-type", "analytic-rts"]);
}
