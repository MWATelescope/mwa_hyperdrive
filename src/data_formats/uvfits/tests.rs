...
use mwa_rust_core::{
    pos::xyz::xyzs_to_cross_uvws_parallel, time::gps_to_epoch, Jones, RADec, XyzGeodetic, UVW,
};
use ndarray::prelude::*;
...
fn test_new_uvfits_is_sensible() {
    ...
    let maps = crate::calibrate::params::generate_tile_baseline_maps(num_tiles, &HashSet::new());
...
fn write_then_read_uvfits(autos: bool) {
    ...
    let flagged_baselines = HashSet::new();
    let flagged_fine_chans = HashSet::new();
    let maps = crate::calibrate::params::generate_tile_baseline_maps(num_tiles, &flagged_tiles);
