...
/// A struct containing an Hour Angle and Declination. As the sine and cosine of
/// these coordinates is often used, these are also stored in the struct. All
...
impl HADec {
    ...
    pub fn to_azel(&self, latitude: f64) -> AzEl {
        let mut az = 0.0;
        let mut el = 0.0;
        unsafe { eraHd2ae(self.ha, self.dec, latitude, &mut az, &mut el) }
        AzEl::new(az, el)
    }
    ...
    pub fn separation(&self, b: &Self) -> f64 {
        unsafe { eraSeps(self.ha, self.dec, b.ha, b.dec) }
    }
}
