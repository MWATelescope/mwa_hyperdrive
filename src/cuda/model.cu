...
__global__ void model_points_kernel(const int num_freqs, const int num_baselines, const FLOAT *freqs, const UVW *uvws,
                                    const Points comps, const JONES *beam_jones, const int *tile_map,
 ...
 */
__global__ void model_shapelets_kernel(const int num_freqs, const int num_baselines, const FLOAT *freqs,
                                       const UVW *uvws, const Shapelets comps, const FLOAT *shapelet_basis_values,
                                       const int sbf_l, const FLOAT sbf_c, const FLOAT sbf_dx, const JONES *beam_jones,
                                       const int *tile_map, const int *freq_map, const int num_fee_freqs,
                                       const int *tile_index_to_unflagged_tile_index_map, JonesF32 *vis) {
...
extern "C" const char *model_shapelets(const Shapelets *comps, const Addresses *a, const UVW *d_uvws,
    ...
    model_shapelets_kernel<<<gridDim, blockDim>>>(a->num_freqs, a->num_baselines, a->d_freqs, d_uvws, *comps,
                                                  a->d_shapelet_basis_values, a->sbf_l, a->sbf_c, a->sbf_dx,
                                                  d_beam_jones, a->d_tile_map, a->d_freq_map, a->num_unique_beam_freqs,
                                                  a->d_tile_index_to_unflagged_tile_index_map, a->d_vis);
