...
fn assert_list_zenith_visibilities(vis: ArrayView2<Jones<f32>>, fail_str: &str) {
    ...
    let expected = Array1::from_elem(vis.dim().0, Jones::identity()).mapv(TestJones::from);
    let result = vis.slice(s![.., 0]).mapv(TestJones::from);
    assert!(
        abs_diff_eq!(expected, result),
        "{fail_str}:\nexpected: {expected:?}\ngot:      {result:?}\n",
    );
    // 3 Jy XX and YY.
    let expected = Array1::from_elem(vis.dim().0, Jones::identity() * 3.0).mapv(TestJones::from);
    let result = vis.slice(s![.., 1]).mapv(TestJones::from);
    assert!(
        abs_diff_eq!(expected, result),
        "{fail_str}:\nexpected: {expected:?}\ngot:      {result:?}\n",
    );
    // 2 Jy XX and YY.
    let expected = Array1::from_elem(vis.dim().0, Jones::identity() * 2.0).mapv(TestJones::from);
    let result = vis.slice(s![.., 2]).mapv(TestJones::from);
    assert!(
        abs_diff_eq!(expected, result),
        "{fail_str}:\nexpected: {expected:?}\ngot:      {result:?}\n",
    );
