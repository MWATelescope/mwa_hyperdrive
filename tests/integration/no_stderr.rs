...
use crate::{solutions_apply::get_1090008640_identity_solutions_file, *};
...
fn test_solutions_apply_no_stderr() {
    ...
    get_1090008640_identity_solutions_file(&sols);
    let args = get_reduced_1090008640(true, false);
    let data = args.data.unwrap();
...
fn test_srclist_shift_no_stderr() {
    ...
    mwa_hyperdrive_common::serde_json::to_writer(&mut f, &shift).unwrap();
    f.flush().unwrap();
    drop(f);
